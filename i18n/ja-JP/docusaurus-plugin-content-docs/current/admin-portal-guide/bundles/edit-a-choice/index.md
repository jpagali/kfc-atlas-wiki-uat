---
title: 選択肢を編集する
sidebar_label: 選択肢を編集する
---

# 選択肢を編集する

## このガイドで扱う内容

このガイドでは、Byte Commerce Admin Portal で選択肢を編集する手順を説明します。

## 手順

**ステップ 1:** まず、こちらをクリックして Bundles 画面に移動します。
**ステップ 2:** on the choices tab をクリックします。

![Step 2](./img/step-2.png)

**ステップ 3:** this  ボタン in the same row the choice you want to edit is in and then click Edit をクリックします。

![Step 3](./img/step-3.png)

**ステップ 4:** Make edits to choice

**ステップ 5:** Hit create choice.

![Step 4](./img/step-4.png)

## 追加情報

- バンドル - 選択肢を編集する
- Search by Bundle Name, Bundle Code, Catalog Tags, Promo Tags
- Optional: Add more variants if needed and go through the same process you did for Step 5

---

*[管理ポータルガイド](/docs/admin-portal-guide) の一部 · セクション: バンドル*